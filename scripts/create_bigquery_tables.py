...
pd.read_csv("/mnt/buckets/liulab/ftp/GSE115978/GSE115978_cell.annotations.csv")\
  .rename(mapper=make_column_name_valid, axis=1)\
...
tpm_unpivoted = tpm\
  .rename_axis("single_cell", axis="columns")\
  .stack()\
  .reset_index(name="expression")\
  [['single_cell', 'gene', 'expression']]
logger.info(f"loading tpm_unpivoted")
tpm_unpivoted.to_gbq("gse115978_tirosh.tpm_unpivoted", chunksize=1_000_000, if_exists="replace")
