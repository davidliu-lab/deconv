...
if __name__ == "__main__":
    ...
    for i, malignant_fraction_log2_fc in enumerate(malignant_fraction_log2_fc_values):
        rng_seed = i + 1
        experiment_name = f"rng_seed={rng_seed:02d},malignant_frac_log2={malignant_fraction_log2_fc:.3f}"
        rng = np.random.default_rng(seed=rng_seed)
        logger.debug("starting experiment %s", experiment_name)
        experiment_path = path_results / experiment_name
        assert np.allclose(fractions_tcga_skcm_mets, perturb_malignant_fractions(fractions_tcga_skcm_mets, 0.0))
        ...
        gene_stats_malignant_cibersortx = compute_stats(rnaseq_malignant_cibersortx, "control", experiment_name)
